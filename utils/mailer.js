...
function baseTemplate(content) {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="#01271a" style="background-color:#01271a;">
    ...
    <table role="presentation" width="480" cellpadding="0" cellspacing="0" bgcolor="#01271a" style="background-color:#01271a;max-width:480px;">
      ...
      <tr><td align="center" bgcolor="#f5efe3" style="background-color:#062318;padding:24px;">
        ${content}
      </td></tr>
...
async function sendOrderConfirmation(order) {
  const html = baseTemplate(`
    ...
    <h2 style="font-family:Georgia,serif;font-size:28px;font-weight:300;color:#062318;margin:0 0 8px">Thank you, ${order.customer.name.split(' ')[0]}.</h2>
    <p style="font-size:15px;color:#7a6040;line-height:1.85;margin:0 0 16px">Your order <strong style="color:#062318">${order.ref}</strong> has been received. We will confirm by email within 24 hours and begin preparing your piece by hand in Egypt.</p>

    <table width="100%" cellpadding="0" cellspacing="0" style="background:#ede3d0;padding:12px 16px;margin-bottom:16px;text-align:left">
      ${orderRows(order)}
      <tr>
        <td style="padding-top:8px;border-top:1px solid rgba(26,18,10,.08);font-size:15px;font-weight:500;color:#062318">Total</td>
        <td style="padding-top:8px;border-top:1px solid rgba(26,18,10,.08);font-size:15px;font-weight:500;color:#062318;text-align:right">${fmt(order.total)}</td>
      </tr>
    </table>
    ...
    <div style="background:#ede3d0;padding:14px 16px;margin-bottom:16px;text-align:left">
...
async function sendAdminNotification(order) {
  const html = baseTemplate(`
    <p style="font-family:Georgia,serif;font-size:20px;font-weight:300;color:#062318;margin:0 0 12px;text-align:left">
      New Order: ${order.ref}
    </p>
    ...
    ${order.customer.notes ? `<p style="font-size:12px;color:#7a6040;margin-top:12px;text-align:left">Notes: ${order.customer.notes}</p>` : ''}
  `);
...
async function sendRequestNotification(type, data) {
  const html = baseTemplate(`
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="text-align:left">
      <tr><td style="font-size:12px;color:#7a6040;padding:4px 0;width:110px">Order Ref</td><td style="font-size:12px;color:#3d2f1f">${data.ref || ''}</td></tr>
      <tr><td style="font-size:12px;color:#7a6040;padding:4px 0">Name</td><td style="font-size:12px;color:#3d2f1f">${data.name || ''}</td></tr>
      <tr><td style="font-size:12px;color:#7a6040;padding:4px 0">Address</td><td style="font-size:12px;color:#3d2f1f">${data.address || ''}</td></tr>
      <tr><td style="font-size:12px;color:#7a6040;padding:4px 0">Reason</td><td style="font-size:12px;color:#3d2f1f">${data.reason || ''}</td></tr>
    </table>
  `);
